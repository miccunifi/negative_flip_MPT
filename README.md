...
Schematic overview of the proposed Margin Preserving Training (MPT) approach. A model ![equation](https://latex.codecogs.com/svg.image?$%5Cphi$) is trained with the margin-calibrated loss ![equation](https://latex.codecogs.com/svg.image?$%5Cmathcal%7BL%7D_%7B%5CDelta%7D$) while receiving double-source focal distillation from two reference models, ![equation](https://latex.codecogs.com/svg.image?$%5Cphi_%7B%5Ctext%7Bold%7D%7D$) and ![equation](https://latex.codecogs.com/svg.image?$%5Cphi_%7B%5Ctext%7Bnew%7D%7D$) (both optimized with cross-entropy). This combination mitigates margin underestimation for new classes, reduces negative flips between model updates, and preserves decision margins for old classes. The gray region marks areas near decision boundaries where negative flips typically occur. Margins are illustrated in a 2D embedding space for clarity, although MPT operates on logit-space margins over all classes.
